...
final class Config
{
    ...
    private static function _getInstance($configTag, $type = self::NORMAL_CLASS_TAG){
        ...
        if(isset(self::$configList[$md5])){
            ...
            $filePath   = APP_PATH . DIRECTORY_SEPARATOR . 'conf' . DIRECTORY_SEPARATOR . $fileName . '.php';

            if(file_exists($filePath)){
                switch($type){
                    case self::NORMAL_CLASS_TAG :
                        //线上配置
                        $prodFilePath = APP_PATH . DIRECTORY_SEPARATOR . 'conf' . DIRECTORY_SEPARATOR . 'production' . DIRECTORY_SEPARATOR . $fileName . '.php';
                    ...
                    case self::YAF_SIMPLE_CLASS_TAG :
                        $className = '\\Yaf\\Conf\\'.ucwords($type);
